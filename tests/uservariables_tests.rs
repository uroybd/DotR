...
"#,
    ...
    if let Some(toml::Value::Table(db_table)) = ctx.get_variable("database") {
        assert_eq!(
            db_table.get("host"),
    ...
    if let Some(toml::Value::Table(api_table)) = ctx.get_variable("api") {
        assert_eq!(
            api_table.get("key"),
